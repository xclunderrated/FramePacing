...
enum PacerMode : std::uint32_t {
    PacerMode_DisplayLocked = 0,  // Console front-edge: VBI phase-locked loop on measured refresh
    PacerMode_Async         = 1,  // RTSS-style post-present wait (compat fallback)
    PacerMode_VrrLive       = 2,  // VRR Live adaptive sync
    PacerMode_LatencyFirst  = 3,  // default: Special K Latent sync (front-edge VBI park + back-edge bias)
};
