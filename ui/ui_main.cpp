...
void show_hamburger_menu(HWND hwnd, int x, int y) {
    ...
    AppendMenuW(bias_menu, MF_STRING, 5101, L"Off (Max Smooth)");
    AppendMenuW(bias_menu, MF_STRING, 5102, L"25%");
    AppendMenuW(bias_menu, MF_STRING, 5103, L"50%");
    AppendMenuW(bias_menu, MF_STRING, 5104, L"75%");
    AppendMenuW(bias_menu, MF_STRING, 5105, L"Max (Low Latency)");
    ...
    UINT bias_flags = (g_current_mode == pacer::PacerMode_LatencyFirst)
                          ? MF_POPUP
                          : (MF_POPUP | MF_GRAYED);
    ...
    else if (cmd == 5101) { g_delay_bias = 0.0;  svc::set_delay_bias(g_delay_bias); }
    else if (cmd == 5102) { g_delay_bias = 0.25; svc::set_delay_bias(g_delay_bias); }
    else if (cmd == 5103) { g_delay_bias = 0.5;  svc::set_delay_bias(g_delay_bias); }
    else if (cmd == 5104) { g_delay_bias = 0.75; svc::set_delay_bias(g_delay_bias); }
    else if (cmd == 5105) { g_delay_bias = 1.0;  svc::set_delay_bias(g_delay_bias); }
